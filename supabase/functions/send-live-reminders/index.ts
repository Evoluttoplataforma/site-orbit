...
// Supabase MKT Orbit (antigo) — onde estão os leads
...
async function fetchLeads(): Promise<Array<{ nome: string; email: string }>> {
  // Busca leads únicos do Supabase antigo (MKT Orbit) onde está a base completa
  const resp = await fetch(
    `${ORBIT_URL}/rest/v1/live_orbit_leads?select=nome,email&order=created_at.desc&limit=1000`,
    {
      headers: {
        apikey: ORBIT_KEY,
...
serve(async (req) => {
  ...
  try {
    const { type, test } = await req.json() as { type: EmailType; test?: boolean };
    ...
    if (test) {
      return new Response(JSON.stringify({ message: "test mode", leads_count: leads.length, type }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    for (const lead of leads) {
      const ok = await sendEmail(lead.email, lead.nome, type);
      if (ok) sent++;
      else errors++;
    ...
    return new Response(JSON.stringify({ success: true, sent, errors, total: leads.length, type }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
